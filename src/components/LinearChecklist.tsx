import { Printer, ArrowLeft, CheckCircle, Circle, Clock, Package, AlertCircle } from 'lucide-react';
import { useAppStore } from '../store';
import { Volume, STATUS_LABELS, Status } from '../types';
import { cn } from '../utils/cn';
import { useMemo } from 'react';

const getPriority = (volume: Volume): number => {
  if (volume.status === 'pending') return 1;
  if (volume.status === 'review') return 2;
  if (volume.status === 'bagging') return 3;
  if (volume.status === 'done') return 4;
  return 5;
};
